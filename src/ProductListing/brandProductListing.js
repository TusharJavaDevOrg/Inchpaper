...
import { fetchSubSubCategoryProductsUrl, getProductsFormBrandIdUrl, getProductsFormCollectionIdUrl, supplierId } from '../../Config/Constants';
...
class BrandProductListing extends Component {
    constructor(props) {
        super(props);
    ...
    async componentDidMount() {
        ...
        var brandId = this.props.route.params?.id;
        var from = this.props.route.params?.from;
        console.log('frommmm', from)
        this.props.deleteAllDefaultVarinats();
        this.fetchAllProductsInThisBrand(brandId, 1);

    }
    componentWillUnmount() {
        this.props.deleteAllDefaultVarinats();
    }
    fetchAllProductsInThisBrand = async (brandId, pageNum) => {
        var url = getProductsFormBrandIdUrl(
            brandId,
            pageNum,
            supplierId,
        );
        ...
        await Axios.get(url, {
            ...
            .then((response) => {
                // console.log(
                //     ' Brand products from server on brand Product Listing',
    ...
    render() {
        ...
        var from = this.props.route.params?.from;
        var brandName = this.props.route.params?.brandName;
        return (
            <View style={{ flex: 1 }}>
                ...
                <Header backgroundColor="#fff"
                    ...
                    centerComponent={{ text: brandName }}
                        ...
                        <View>
                            {this.state.isProductLoading ?
                                ...
                                : <FlatList data={this.state.allProducts}
                                    ListFooterComponent={() => this.state.bottomOneLoader ? <ActivityIndicator size={'small'} color={Colors.primary} style={{ alignItems: 'center', alignSelf: 'center' }} /> : null}
                                    onEndReached={() => {
                                        if (this.state.bottomOneLoader) {
                                            const { id } = this.props.route.params;

                                            this.fetchAllProductsInThisBrand(
                                                id,
                                    ...
                                    numColumns={2} keyExtractor={(item, index) => String(index)} renderItem={({ item, index }) => {
                                        ...
                                        // console.log('collection products', JSON.stringify(item))
                                        return (
                                            <View style={{ width: Dimensions.get('window').width / 2.1, paddingTop: 10, borderColor: '#efefef', borderWidth: 0.5, padding: 5 }}>
                                                <TouchableOpacity onPress={() => this.props.navigation.navigate('ProductDisplay', { product: item, from: 'productListing' })} style={{ width: Dimensions.get('window').width / 2.1, paddingTop: 10, borderColor: '#efefef', borderWidth: 0.5, padding: 5 }} >
                                                    <ImageBackground borderRadius={10} source={{ uri: item?.medias[0]?.mediaUrl }} style={{ height: 130, width: '100%' }}>
                                                        <View style={{ alignSelf: 'flex-end', padding: 8 }}>
                                                    ...
                                                    <View style={{ padding: 5 }}>
                                                        ...
                                                        {item.productListings[0].mrp - item.productListings[0].sellingPrice > 0 && <Text style={{ paddingLeft: 5, fontFamily: 'sans-serif-light', fontWeight: 'bold', fontSize: 12, color: '#000', }}>Save ₹ {Math.round(item.productListings[0].mrp
                                                        ) -
...
export default connect(mapStateToProps, mapDispatchToProps)(BrandProductListing)
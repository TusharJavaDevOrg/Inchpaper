...
import { fetchFeaturesProductsUrl, fetchSubSubCategoryProductsUrl, getProductsFormBrandIdUrl, getProductsFormCollectionIdUrl, supplierId } from '../../Config/Constants';
...
class FeaturedProducts extends Component {
    ...
    async componentDidMount() {
        ...
        this.fetchFeaturedProducts(1);

    }
    componentWillUnmount() {
        this.props.deleteAllDefaultVarinats();
    }
    fetchFeaturedProducts = async (pageNum) => {
        var url = fetchFeaturesProductsUrl(pageNum);
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                <Header backgroundColor="#fff"
                    ...
                    centerComponent={{ text: 'Featured Products' }}
                            ...
                            {this.state.isProductLoading ?
                                ...
                                : <FlatList data={this.state.allProducts}
                                    ...
                                    ListFooterComponent={() => this.state.bottomOneLoader ? <View>
                                        <ActivityIndicator size={'small'} color={Colors.primary} style={{ justifyContent: 'center', alignSelf: 'center' }} />
                                    </View> : null}
                                    numColumns={2} keyExtractor={(item, index) => String(index)} renderItem={({ item, index }) => {
